...
import static io.userhabit.kongsuny.common.Config.EXCEL_FILE_NAME;
...
public class ExcelGenerater {
    ...
    private static final int COLUMN_INDEX_UNIQUE_ID = 8;
    private static final int COLUMN_INDEX_CREATE_DATE = 9;
    ...
    public File saveExcelFile(List<AppInfoModel> list) {
        ...
        try {
            ...
            mExcelFile = new File(PARENT_DIR,cal.getTimeInMillis() + EXCEL_FILE_NAME);
            ...
            c = row.createCell(COLUMN_INDEX_UNIQUE_ID);
            c.setCellValue("고유 ID");
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                c = row.createCell(COLUMN_INDEX_UNIQUE_ID);
                c.setCellValue(item.getUniqueId());
                c = row.createCell(COLUMN_INDEX_CREATE_DATE);
            ...
            sheet.autoSizeColumn(COLUMN_INDEX_UNIQUE_ID);
